...
export default function WeightMaster() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await createMutation.mutateAsync({ bean_type_id: beanTypeId, weight: weightNum })
      toast.success('Weight master created successfully')
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await updateMutation.mutateAsync({
        id: editingId,
        data: { bean_type_id: beanTypeId, weight: weightNum },
      })
      toast.success('Weight master updated successfully')
      setEditingId(null)
      setBeanTypeId('')
      setWeight('')
    } catch (error: unknown) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.success('Weight master deleted successfully')
    } catch (error: unknown) {
      const message =
        error instanceof Error && 'response' in error
  ...
  const openEditModal = (id: string, currentBeanTypeId: string, currentWeight: number) => {
    setEditingId(id)
    setBeanTypeId(currentBeanTypeId)
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Weight Master — Manage bean type weights</p>
        </div>
        <button
      ...
      <div className="card">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by bean type name..."
      ...
      <div className="card overflow-hidden p-0">
        {isLoading ? (
          ...
          <div className="px-4 py-8 text-center text-gray-500">No weight records found</div>
        ) : (
          <>
            {/* Desktop table */}
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {weightList?.map((wm) => (
                    <tr key={wm.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className="table-cell font-medium">{wm.bean_type_name || '-'}</td>
                      <td className="table-cell">{wm.weight}</td>
                      <td className="table-cell text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => openEditModal(wm.id, wm.bean_type_id, wm.weight)}
              ...
              {weightList?.map((wm) => (
                <div key={wm.id} className="p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-gray-900 dark:text-white">{wm.bean_type_name || '-'}</span>
